...
public class Exam0510 {
  public static void main(String[] args) {
    int[] p  = new int[3]; // c 언어 => (int*)malloic(sizeof(int) * 3)
    // p
    // - int 배열의 주소를 담는 변수이다
    // - 자바에서는 주소를 담는 변수를 '레퍼런스'라고 부른다.
    //
    // new int[3]
    ...
    p2 = p; // p 레퍼런스에 저장 된 주소를 p2 래퍼런스에 복사한다.
    p2[1] = 1000;
    ...
    //  System.out.println("p3[1]");  // 컴파일 오류! 존재하지 않음.
    
    //레퍼런스 변수의 주소 값을 0으로 초기화하기
    //p2 = 0 ; //컴파일 오류! 이렇게 직접적으로 초기화시킬 수 없다.
//    p2 = null; // 주소 변수(래퍼런스)를 0으로 초기화시키려면 null을 사용하라.
//    
//    System.out.printf("%d\n",p2[0]);
...
//
    ...
    // ** [] 안에 배열의 개수를 지정해서는 안된다.
    
    
    int k4[];
    k4 = new int[] {10, 20};
    
    int[] k5 = {11, 12, 13}; // new int를 생략해도됨.
    
    // int[] k6;
    // k6 = {11, 12, 13}; // 컴파일 오류! 변수 선언 없이는 생략할 수 없다.
    
    int[] x1 = new int[3];
    int[] x2 = new int[4];
    int[] x3 = x1;
    x2 = x1; //x2가 가리켯던 int[4] 배열 메모리의 주소를 잃어버려
              //더 이상 사용할 수 없다.
              //이렇게 주소를 잃어 버려 사용할 수 없는 메모리를 Garbage라 한다.
      // 가비지는 JVM이 자동으로 해제시킨다.
      // 단 바로 해제시키는 것이 아니라 ㅜ다음 조건에 해당될 때 해제시킨다.
      // 1) 메모리가 부족할 때
      // 2) 시스템이 한참동안 한가할 때
      // = > 결론! 언제 가비지를 메모리에서 해제시킬지 알 수 없다.
      //
    //System.gc(); 가능하다면 빠른 시일 내에 가비지를 수정해 줄 것을 요구!
    // 주의
    // => gc()를 호출한다고 해서 바로 가비지 컬렉터가 동작하는 것은 아니다.
    // => 가비지 컬렉터의 실행을 앞당겨 달라고 요청하는 것이다.
    // => 그 요구를 수용할지 여부는 자바버추얼머신 판단에 달려있따.
    // => 따라서 gc()를 호출하면 가비지 컬렉터가 바로 출력한다고 믿지말라!
